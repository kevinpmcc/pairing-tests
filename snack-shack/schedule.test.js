...
test.only('schedule#getSchedule returns the correct schedule when four sandwich orders have been placed', () => {
  ...
  test.only('schedule#createSteps returns the correct number of steps', () => {
    ...
    expect(app.createSteps([{ orderItem: 'fakeitem1' }, { orderItem: 'fakeitem1' }], fakeTimings).length).toEqual(4)
  })

  test.only('schedule#createSteps returns the correct number of steps', () => {

    expect(app.createSteps([{ orderItem: 'fakeitem1' }, { orderItem: 'fakeitem2' }], fakeTimings).length).toEqual(6)
  })

  test.only('returns all necessary elements in steps', () => {

    expect(app.createSteps([{ orderItem: 'fakeitem1' }], fakeTimings)).toEqual(
  ...
  test.only('returns all necessary elements in steps', () => {
    ...
    expect(app.createSteps([{ orderItem: 'fakeitem1' }, { orderItem: 'fakeitem1'}], fakeTimings)).toEqual(
      [ { name: 'fakestep1', orderItem: 'fakeitem1', duration: 5, orderItemNumber: 1, startTime: 0 },
        { name: 'fakestep2', orderItem: 'fakeitem1', duration: 60, orderItemNumber: 1, startTime: 5 },
  ...
  test.only('returns all necessary elements in steps', () => {
    ...
    expect(app.createSteps([{ orderItem: 'sandwich' }, { orderItem: 'sandwich' },{ orderItem: 'sandwich' }], timings)).toEqual(
